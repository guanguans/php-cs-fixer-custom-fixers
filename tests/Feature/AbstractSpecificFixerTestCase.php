 ...
 */
abstract class AbstractSpecificFixerTestCase extends TestCase
{
    ...
    final public function testIsRisky(): void
    {
        if ($this->fixer->isRisky()) {
            $this->assertValidDescription($this->fixer->getName(), 'risky description', $this->fixer->getDefinition()->getRiskyDescription());
        } else {
    ...
    final public function testFixerDefinitions(): void
    {
        ...
        $this->assertValidDescription($fixerName, 'summary', $definition->getSummary());

        if (null !== $definition->getDescription()) {
            $this->assertValidDescription($fixerName, 'description', $definition->getDescription());
        }
    ...
    final public function testFixersAreFinal(): void
    {
        if ($this->getFixerReflection()->isAnonymous()) {
            self::markTestSkipped('Anonymous fixers cannot be final.');
        }
    ...
    final public function testFixerConfigurationDefinitions(): void
    {
        ...
        foreach ($configurationDefinition->getOptions() as $option) {
            ...
            $this->assertOption($option, $this->fixer);
        }
    }
    ...
    {
        if (!$this->fixer instanceof FixerInterface) {
    ...
    private function assertOption(FixerOptionInterface $option, FixerInterface $fixer): void
    {
        ...
        $this->assertValidDescription($fixer->getName(), 'option:'.$option->getName(), $option->getDescription());
    ...
    private function findAllTokenSequences(Tokens $tokens, array $sequence): array
    {
        ...
        while (null !== ($found = $tokens->findSequence($sequence, $lastIndex))) {
